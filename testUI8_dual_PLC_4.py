    ...
    def run(self):
        ...
        while self.running:
            ...
            if frame is not None:
                # detect_objects 可能返回 3 或 4 个值，兼容处理
...
class MyWindow(QWidget):
    ...
    def start_detection(self):
        ...
        # 优先尝试建立并复用 socket 连接，降低发送失败和重连延迟
        try:
            self._connect_robot()
        except Exception:
            pass
    ...
    def _send_initial_empty_frames(self, count=50, interval=0.12):
        def worker():
            ...
            try:
                # 尝试复用已打开的 socket_client，否则新建一个持久连接复用
                if getattr(self, "socket_client", None):
                    ...
                    try:
                        ...
                        print("初始空帧：建立持久连接失败，改为每次短连接发送:", e)
                        try:
                            client.close()
                        except:
                            pass
                ...
                for i in range(count):
                    try:
                        # 线程安全获取并自增 frame_num，确保前50次与主程序连续一致
                        with self._frame_num_lock:
                            cur_frame_num = self.frame_num
                            self.frame_num += 1

                        ts = int(time.time() * 1000)
                        data_header = f"Data;{cur_frame_num};{ts};{0};"
                        full_data = "STX" + data_header + "ETX"
                        ...
                        if client:
                            try:
                                ...
                                try:
                                    ...
                                    pass
                                client = None
                                print(f"持久连接发送失败，尝试短连接重发（序号{i}）：", oe)
                            ...
                            try:
                                ...
                                print(f"临时连接发送失败(序号{i}):", e)
                            finally:
                                try: tmp.close()
                                except: pass
                    except Exception as e:
                        print(f"初始空帧发送出错（序号 {i}）：{e}")
                    # 保持每次发送间隔约 interval
                    time.sleep(interval)
            finally:
    ...
    def _continue_start_detection(self):
        """在初始空帧发送完成后真正开始检测（在主线程中执行）"""
        # 禁用 CameraThread 的直接画面发射，改由 YoloThread 发画面
