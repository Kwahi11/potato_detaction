...
def send_mw3_value(value, plc_enabled=True):
    print(f"🛰️ 写入 PLC: %MW3 = {value}")
    if not plc_enabled:
        print("🚫 模拟模式：未发送，仅打印")
        return
...
class MyWindow(QWidget):
    def __init__(self):
        ...
        self.mw3_sequence = [1, 2, 1 ]
        self.mw3_index = 0
        self.mw3_detection_started = False

        # 启动 PLCWriter
        self.plc_writer = PLCWriter(ip="192.168.1.88", port=502, addr=3)

        # 定时器1：空闲时发送0
        self.mw3_idle_timer = QTimer(self)
        self.mw3_idle_timer.timeout.connect(self._mw3_send_idle)

        # 定时器2：检测中发送1/2
        self.mw3_work_timer = QTimer(self)
        self.mw3_work_timer.timeout.connect(self._mw3_send_work)

        # 初始启动空闲定时器
        self.mw3_idle_timer.start(500)

    def _mw3_send_idle(self):
        # 空闲状态定时发0
        self.plc_writer.write_value(0)
        print("🛰️ 空闲模式: 写入 PLC: %MW3 = 0")

    def _mw3_send_work(self):
        # 检测状态交替发1/2
        value = self.mw3_sequence[self.mw3_index]
        self.mw3_index = (self.mw3_index + 1) % len(self.mw3_sequence)
        self.plc_writer.write_value(value)
        print(f"🛰️ 检测模式: 写入 PLC: %MW3 = {value}")
    ...
    def start_detection(self):
        ...
        # self.mw3_detection_started = True
        # self.mw3_index = 0  # 重置写入序列下标
        # global plc_mw3_flag
        # plc_mw3_flag = 1
        self.mw3_idle_timer.stop()
        self.mw3_work_timer.start(500)
        print("🚀 开始检测")
    ...
    def stop_detection(self):
        ...
        # global plc_mw3_flag
        # plc_mw3_flag = 0  # 发送0
        self.mw3_work_timer.stop()
        self.mw3_idle_timer.start(500)

        print("🛑 停止检测，恢复写入 MW3 = 0")
    ...
    def update_detections(self, detections, centroids,current_time):
        ...
        self._send_data_frame(centroids,current_time)  # 发送数据帧
...
class PLCWriter:
    def __init__(self, ip="192.168.1.88", port=502, addr=3):
        self.ip = ip
        self.port = port
        self.addr = addr
    ...
    def write_value(self, value):
        print(f"🛰️ 写入 PLC: %MW{self.addr} = {value}")
        if not self.connected:
            print("❌ 未连接 PLC")
            return
        result = self.client.write_register(self.addr, value)
        if result.isError():
            print(f"❌ 写入失败: %MW{self.addr} = {value}")
        else:
            print(f"✅ 成功写入: %MW{self.addr} = {value}")
