...
class CameraThread(QThread):
    ...
    def run(self):
        ...
        while self.running:
            ...
            for ret, frame,camera_time in retrun_frame():
                ...
                if ret:
                    ...
                    if self.emit_frame:  # 如果允许发出信号
                        self.frame_signal.emit(self.current_frame,self.camera_time)  # 发送画面
...
class MyWindow(QWidget):
    ...
    def _send_data_frame(self, centroids,camera_time):
        """发送数据帧，刀具角度在第七位"""
        ...
        try:
            ...
            trigger_time = int(camera_time)  # 时间戳
    ...
    def update_detections(self, detections, centroids,camera_time):
        """更新检测结果"""
        bug_centers = []
        ...
        self._send_data_frame(centroids,camera_time)  # 发送数据帧
